...
func normalize(x uint64) uint64 {
	...
	if x >= uint64(1000000000000000000) {
		return uint64(1000000000000000000)
	}
	return uint64(x)
}

func New(metaData *model2.Meta, config *config.Config, sc *Score) Scaler {
...
func (s *Simple) Assign(ctx context.Context, request *pb.AssignRequest) (*pb.AssignReply, error) {
	...
	initUsage := uint64(time.Now().UnixMilli()-start.UnixMilli())
	atomic.AddUint64(&s.sc.sumT, initUsage)
	s.initSum += initUsage
	s.initTimes += 1
	return &pb.AssignReply{
		Status: pb.Status_Ok,
...
func (s *Simple) Idle(ctx context.Context, request *pb.IdleRequest) (*pb.IdleReply, error) {
	...
	atomic.AddUint64(&s.sc.sumR, request.Result.DurationInMs*normalize(s.metaData.MemoryInMb))
	atomic.AddUint64(&s.sc.sumT, request.Result.DurationInMs)
	atomic.AddUint64(&s.sc.realR, request.Result.DurationInMs*normalize(s.metaData.MemoryInMb))
	atomic.AddUint64(&s.sc.realT, request.Result.DurationInMs)
	...
	for range ticker.C {
		for {
			...
			if element := s.idleInstance.Back(); element != nil {
				...
				realR:=normalize(atomic.LoadUint64(&s.sc.realR))
				sumR:=normalize(atomic.LoadUint64(&s.sc.sumR))
				realT:=normalize(atomic.LoadUint64(&s.sc.realT))
				sumT:=normalize(atomic.LoadUint64(&s.sc.sumT))
				// big E have long wait time
				E := normalize(s.initSum) / normalize(s.initTimes)
				// small q have long wait time
				q := uint64(float64(idleDuration)*0.05)
	
				scoreWait := (float64(realR)/float64(sumR+(uint64(s.idleInstance.Len())*q*normalize(s.metaData.MemoryInMb))))+(float64(realT)/float64(sumT))
				scoreNoWait := (float64(realR)/float64(sumR))+(float64(realT)/float64(sumT+E))

				if scoreWait < scoreNoWait {
					atomic.AddUint64(&s.sc.sumR, uint64(idleDuration)*normalize(s.metaData.MemoryInMb))
					//need GC
					s.idleInstance.Remove(element)
					delete(s.instances, instance.Id)
					s.mu.Unlock()
					go func() {
						reason := fmt.Sprintf("Idle duration: %dms, excceed configured duration: %fs", idleDuration, s.config.IdleDurationBeforeGC.Seconds())
